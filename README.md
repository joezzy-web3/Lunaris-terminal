...
- **Rejected Trades Archive (Zero Data Loss):** Ingestion-rejected events (e.g., test harness tags or early format spikes) are never silently purged or injected back into trading history. Instead, they are preserved in an isolated archive.
- **Downloadable for Judges:** Judges can click the red **`Rejected Trades`** button directly in the Audit View (next to *Copy JSON*) to inspect each rejected record, verify its rejection reason and SHA-256 hash, or download the raw data as `.JSON` or `.CSV`.
