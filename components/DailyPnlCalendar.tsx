...
export const DailyPnlCalendar: React.FC<DailyPnlCalendarProps> = ({
  ...
  // Compact number formatter for calendar pills (+1.35K, -30.99, +820.99)
  const formatPnlCompact = (val: number): string => {
    ...
    if (absVal >= 1000) {
      const kVal = (absVal / 1000).toFixed(2);
      // Remove trailing zero if .x0 (e.g. 1.80 -> 1.8) or .00 (e.g. 1.00 -> 1)
      const cleanK = kVal.replace(/\.00$/, '').replace(/(\.[0-9])0$/, '$1');
      return `${sign}${cleanK}K`;
    } else {
