        ...
        {activeTab === 'TERMINAL' && (
          <div className="space-y-4 animate-fadeIn" key={resetKey}>
            ...
            <div className="bg-[#0b0b10] border border-white/10 rounded-xl p-2.5 shadow-2xl">
              ...
              {/* Symbol & Name Module Pills — Uniform Pulse-style buttons across all modules */}
            ...
            {cockpitModule === 'STREAM' && (
              <div className="space-y-4 animate-fadeIn">
                <AgentActivityStream
                  selectedTicker={councilSelectedTicker}
                  onJumpTo={(mod, asset, extraInfo) => {
                    const normAsset = asset ? normalizeTickerSymbol(asset) : councilSelectedTicker;
                    if (asset) setCouncilSelectedTicker(normAsset);
                    if (mod === 'COUNCIL') {
                      setActivityAdvisoryMandate({
                        ticker: normAsset,
                        mandate: extraInfo || `[Advisory Review]: Reviewing ${normAsset} for Council quorum deliberation.`,
                    ...
                    setCockpitModule(mod as any);
                  }}
                  onStageAdvisoryTrade={(proposal) => {
                    setCouncilSelectedTicker(normalizeTickerSymbol(proposal.asset));
                    handleSendToAutopilot(proposal);
                  }}
                  onHandoffToCouncil={(ticker, advisoryPrompt) => {
                    const norm = normalizeTickerSymbol(ticker);
                    setCouncilSelectedTicker(norm);
                    setActivityAdvisoryMandate({
                      ticker: norm,
                      mandate: advisoryPrompt,
                      source: 'ACTIVITY_STREAM',
                      timestamp: Date.now(),
                    });
                    setCockpitModule('COUNCIL');
                  ...
                  onImportAsset={(ticker) => {
                    setCouncilSelectedTicker(normalizeTickerSymbol(ticker));
                  }}
                />
              </div>
            )}
